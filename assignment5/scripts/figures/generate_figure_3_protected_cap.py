...
def generate_figure_3_peak_dt(results):
    ...
    for i, (cap, dt) in enumerate(zip(cap_values, peak_dt_values)):
        ...
        if dt_rounded not in seen_values:
            ...
            if i > 0 and abs(peak_dt_values[i-1] - dt) < 0.001:
                offset_y = -annotation_offset
            elif i < len(cap_values) - 1 and abs(peak_dt_values[i+1] - dt) < 0.001:
                offset_y = -annotation_offset
    ...
    ax.set_title('Figure 3: Peak DT vs. PROTECTED cap (E2 EDE)', fontweight='bold', pad=15, fontsize=16)
    ...
    ax.text(optimal_cap, y_max * 1.015, f'Optimal (cap = {optimal_cap:.1f})', 
