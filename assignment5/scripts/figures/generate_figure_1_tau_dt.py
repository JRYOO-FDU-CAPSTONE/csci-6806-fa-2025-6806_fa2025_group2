...
def generate_figure_1_peak_dt(results):
    ...
    ax.plot(tau_values, peak_dt_values, 
    ...
    for i, (tau, dt) in enumerate(zip(tau_values, peak_dt_values)):
        ...
        if dt_rounded not in seen_values:
            ...
            elif i < len(tau_values) - 1 and abs(peak_dt_values[i+1] - dt) < 0.05:
                offset_y = -annotation_offset - 5
    ...
    ax.set_title('Figure 1: Peak DT vs. τ_DT (E1 DT-SLRU)', fontweight='bold', pad=20, fontsize=16)
    ...
    if baseline_tau in tau_values:
        ...
        # Use transform to position relative to axes (0-1), then convert to data coordinates
        ax.text(baseline_tau, y_max_global, 'Baseline (τ_DT = 1.0)', 
    ...
    plt.savefig(output_dir / 'figure_1_peak_dt_tau_dt.png', dpi=300, bbox_inches='tight')
    plt.savefig(output_dir / 'figure_1_peak_dt_tau_dt.pdf', bbox_inches='tight')
    plt.close()
    
    print(f"Figure 1 saved to {output_dir}/figure_1_peak_dt_tau_dt.png/.pdf")

def main():
    print("Assignment 5 Figure 1 Generation")
    print("="*50)
    ...
    print("  - assignment5/report/figures/figure_1_peak_dt_tau_dt.png/.pdf")
